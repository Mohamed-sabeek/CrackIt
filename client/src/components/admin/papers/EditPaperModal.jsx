...
const EditPaperModal = ({ isOpen, onClose, onSave, saving, paper }) => {
  ...
  useEffect(() => {
    if (paper) {
      setFormData({
        title: paper.title || '',
        exam: paper.exam || 'TNPSC Group 1',
        stage: paper.stage || 'Prelims',
        paperType: paper.paperType || 'General Studies',
        ...
        >
          <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Edit Paper</h2>
            </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
              ...
              <button 
                type="submit" 
                disabled={saving}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-white bg-amber-500 hover:bg-amber-600 disabled:opacity-50 transition-all"
              >
                {saving ? 'Saving...' : <><Save size={18} /> Update Paper</>}
              </button>
            </div>
          </form>
        </motion.div>
